...
import com.example.playground.domain.usecase.PeopleUseCase
...
@HiltViewModel
class PeopleViewModel @Inject constructor(private var peopleUseCase: PeopleUseCase) : ViewModel() {
    private val _resultPeople = Channel<Result<List<People>>>(Channel.BUFFERED)
    val resultPeople = _resultPeople.receiveAsFlow()

    fun getPeople() {
        viewModelScope.launch { peopleUseCase.getPeople().collect { _resultPeople.send(it) } }
    }
}
