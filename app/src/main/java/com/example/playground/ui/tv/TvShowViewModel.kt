...
import com.example.playground.domain.usecase.TvShowUseCase
...
@HiltViewModel
class TvShowViewModel @Inject constructor(private var tvShowUseCase: TvShowUseCase) : ViewModel() {
    private val _resultTvShow = Channel<Result<List<TvShow>>>(Channel.BUFFERED)
    val resultTvShow = _resultTvShow.receiveAsFlow()

    fun getTvShow() {
        viewModelScope.launch { tvShowUseCase.getTvShow().collect { _resultTvShow.send(it) } }
    }
}
